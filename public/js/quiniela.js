...
for (let i = 0; i < totalPartidos; i++) { const seleccion=document.querySelector(`input[name="resultados[${i}]"
    ]:checked`); if (!seleccion) { Swal.fire('Faltan resultados', 'Selecciona una opción para todos los partidos.'
    , 'warning' ); return; } resultados.push(seleccion.value); } const quiniela={ numero, nombre, telefono, resultados
    }; quinielas.push(quiniela); actualizarResumen(); mostrarQuinielas(); limpiar(); console.log('✅ Quiniela agregada:',
    quiniela); } // Generar resultados aleatorios function aleatorio() { const
    totalPartidos=document.querySelectorAll('tbody tr').length; for (let i=0; i < totalPartidos; i++) { const
    radios=document.querySelectorAll(`input[name="resultados[${i}]" ]`); const random=Math.floor(Math.random() *
    radios.length); radios[random].checked=true; } } // Limpiar selección de radios function limpiar() {
    ...
    mensaje += "\n💳 Pago rápido:\n";
    mensaje += "https://link.mercadopago.com.mx/quinielazas\n";

    mensaje += "\n🏦 O transferencia:\n";
    mensaje += "\nCLABE: 722969069986621442, Beneficiario: Cynthia Eileen Gonzalez Perez, Institución: Mercado Pago W ";
    mensaje += "\n😊 Por favor realiza el pago de tus quinielas y envía tu comprobante con tu nombre.\n";
